...
class UIModelCreator(UIElement):
    ...
    def _get_config_from_inputs(self):
        """
...
class UIFileBrowser(UIElement):
    ...
    def _validated_save_filename(self, filename):
        """
    ...
    def _get_load_file_path(self, filename):
        """
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if event.ui_element == self.ok_button:
                ...
                if self.mode.__contains__("save"):
                    validated = self._validated_save_filename(filename)
                    if validated:
                    ...
                    filepath = self._get_load_file_path(filename)
                    if filepath:
                        if self.directory == "Tracks":
                            r.game_core.current_track = filename
                        else:
...
class UITrackCanvas(UIElement):
        ...
        if (event.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP, pygame.MOUSEMOTION)):
            ...
            if (self.rect.collidepoint(event.pos) and
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    self._process_mouse_down(relative_mouse_pos)

                if event.type == pygame.MOUSEMOTION:
                    self._process_mouse_motion(relative_mouse_pos)
            ...
            if not self.selected_point or self.selected_point[0] == 'c' or len(self.anchor_points) <= 2:
                return
            self._handle_point_delete()

        outer_wall_points, inner_wall_points = self._generate_track()
        self.rebuild(outer_wall_points, inner_wall_points)

    def _process_mouse_down(self, relative_mouse_pos):
        """
        ...
        is_point_selected = self._point_selection_handling(relative_mouse_pos)
        ...
        if is_point_selected:
            ...
            if self.mode == "anchor":
                insertion_index = self._handle_anchor_insertion(relative_mouse_pos)
                if insertion_index is not None:
                    insertion_index += 1
                    self._insert_anchor(relative_mouse_pos, insertion_index)
                else:
                    self._create_new_anchor(relative_mouse_pos)
                self._check_track_completion()
            if self.mode == "width":
                self._create_width_point(relative_mouse_pos)

    def _process_mouse_motion(self, relative_mouse_pos):
        """
        ...
        if self.is_dragging:
            ...
            if self.selected_point[0] == 'a':
                self._handle_anchor_point_movement(relative_mouse_pos, point_index)
                self._check_track_completion()
    ...
    def _point_selection_handling(self, mouse_pos):
        """
    ...
    def _handle_anchor_insertion(self, mouse_pos):
    ...
    def _create_new_anchor(self, mouse_pos):
        ...
        if len(self.anchor_points) > 1:
            ...
            p3 = mouse_pos
            if self.is_handles_enabled:
                self.control_points.extend(self._get_bezier_points(p0, p1, p2, p3))
    ...
    def _insert_anchor(self, mouse_pos, insert_index):
        ...
        for i in range(len(self.anchor_points) - 1):
            ...
            b1, b2 = self._get_bezier_points(p0, p1, p2, p3)
    ...
    def _create_width_point(self, mouse_pos):
    ...
    def _handle_anchor_point_movement(self, mouse_pos, point_index):
        """
    ...
    def rebuild(self, outer_wall_points, inner_wall_points):
        ...
        self._draw_track(outer_wall_points, inner_wall_points)
        if len(self.anchor_points) != 0:
            if self.mode == "anchor":
                self._draw_anchors_and_handles()
            else:
                self._draw_width_points()

        validity_overlay = self._generate_validity_overlay(outer_wall_points, inner_wall_points)
    ...
    def _draw_track(self, outer_wall_points, inner_wall_points):
    ...
    def _draw_anchors_and_handles(self):
    ...
    def _generate_validity_overlay(self, outer_wall_points, inner_wall_points):
        """
        ...
        invalid_walls = self._get_validity_issues(outer_wall_points, inner_wall_points)
        for wall in invalid_walls:
            r.draw_line(validity_overlay, (255, 0, 0, 150), wall[0], wall[1], 15)
        return validity_overlay

    def _hash_grid(self, outer_wall_points, inner_wall_points):
        """
    ...
    def _generate_all_controls(self):
        """
    ...
    @staticmethod
    def _get_bezier_points(p0, p1, p2, p3):
        """
    ...
    def _get_validity_issues(self, outer_wall_points, inner_wall_points):
        """
        ...
        Args:
            outer_wall_points, inner_wall_points (list(Vector2)): wall points
        """
        self._hash_grid(outer_wall_points, inner_wall_points)

        def _check_wall_validity(wall_points, hashed_grid_start_index):
        ...
        invalid_outer_walls = _check_wall_validity(outer_wall_points, 0)
        invalid_inner_walls = _check_wall_validity(inner_wall_points, len(outer_wall_points))
        ...
        elif len(outer_wall_points) < 200 or len(inner_wall_points) < 200:
