...
class NeuralNetwork:
    ...
    def update_params(self, d_mu, d_log_std = None):
        ...
        for i, layer in enumerate(self.layers):
            self.adam_moments['w'][i], weight_update = self._adam_estimation(self.adam_moments['w'][i][0],
                                                                             self.adam_moments['w'][i][1],
                                                                             weight_grads[i], self.adam_moments['t'],
                                                                             self.alpha, self.beta1, self.beta2)

            self.adam_moments['b'][i], bias_update = self._adam_estimation(self.adam_moments['b'][i][0],
                                                                           self.adam_moments['b'][i][1], bias_grads[i],
                                                                           self.adam_moments['t'], self.alpha,
                                                                           self.beta1, self.beta2)
            layer.weights += weight_update
            layer.biases += bias_update

        if d_log_std is not None:
            self.adam_moments['log_std'], log_std_update = self._adam_estimation(self.adam_moments['log_std'][0],
                                                                                 self.adam_moments['log_std'][1],
                                                                                 d_log_std, self.adam_moments['t'],
                                                                                 self.alpha, self.beta1, self.beta2)
    ...
    @staticmethod
    def _adam_estimation(m, v, grad, t, alpha, beta1, beta2):
        """
