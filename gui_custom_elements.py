...
from pygame import Surface, Rect, draw, SRCALPHA, transform
...
class UIGaugeMeter(UIElement):
    ...
    def __init__(self, relative_rect, manager,
        ...
        self.image = Surface((self.relative_rect.width + border_width,
                                     self.relative_rect.height + border_width), SRCALPHA)
        self.rebuild()
    ...
    def rebuild(self):
        ...
        arc_rect = Rect(0, 0, radius * 2, radius * 2)
        arc_rect.midbottom = center
        draw.arc(self.image, self.border_colour, arc_rect, math.pi, 2* math.pi , self.border_width)
        ...
        draw.line(self.image, self.dial_colour, center, end_pos, self.dial_thickness)
...
class UITrackCanvas(UIElement):
    def __init__(self, relative_rect, manager, bg_colour = "White"):
        ...
        self.image = Surface((self.relative_rect.width, self.relative_rect.height), SRCALPHA)
        self.rebuild()
    ...
    def rebuild(self):
        ...
        if self.is_dragging:
            for i in range(len(self.current_stroke) - 1):
                draw.line(self.image, "Red", self.current_stroke[i], self.current_stroke[i+1], 3)
