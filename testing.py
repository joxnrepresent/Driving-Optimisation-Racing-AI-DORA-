...
# Import your adam_estimation function here if needed
def adam_estimation(m, v, grad, t, alpha = 0.1, beta1 = 0.9, beta2 = 0.999, eps = 1e-8):
...
# Simulated gradient pattern (like oscillating RL gradient)
grads = np.array([np.sin(i/3) for i in range(1, 51)])
...
for t, g in enumerate(grads, start=1):
    (m, v), update = adam_estimation(m, v, g, t)
    updates.append(update)
    m_values.append(m)
    v_values.append(v)
# Plot results
plt.figure(figsize=(10, 6))
plt.plot(grads, label='Gradient (input)')
plt.plot(m_values, label='Momentum (1st moment)')
plt.plot(np.sqrt(v_values), label='RMS of 2nd moment')
plt.plot(updates, label='Adam Update')
plt.title("Adam Optimiser Dynamics Over Time")
